...
final aiwriteOrderControllerProvider = Provider.autoDispose((ref) {
  final controller = TextEditingController(
    text:
        "글 구성은 아래 구조를 반드시 따른다. 1. 서론 (약 300~500자) 2. 본문 섹션 4~5개   - 각 섹션마다 소제목 포함   - 섹션당 400~600자 분량   3. 결론 (약 300~500자) 문체는 정보형 블로그 스타일로, 검색 유입을 고려해 자연스럽게 핵심 키워드를 반복 사용한다. 불필요한 군더더기 표현은 줄이고, 실제 사람이 쓴 글처럼 작성한다.",
  );
  ref.onDispose(() => controller.dispose());
  return controller;
});
